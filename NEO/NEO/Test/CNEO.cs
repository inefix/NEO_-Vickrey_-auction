...
namespace CNEO
{
    ...
    public class CNEO : SmartContract
    {
        ...
        public static bool MintTokens()
        {
            ...
            //Increase the total amount of contract assets ---> delete
        ...
        /// </summary>
        [DisplayName("refund")]
        public static int Refund(byte[] from)
        {
            if (from.Length != 20)
            ...
            if (preRefund.AssetId.AsBigInteger() != AssetId.AsBigInteger()) return 1;

            //Not to itself, failed
            if (preRefund.ScriptHash.AsBigInteger() != ExecutionEngine.ExecutingScriptHash.AsBigInteger()) return 2;

            //double refund
            StorageMap refund = Storage.CurrentContext.CreateMap(nameof(refund));
            if (refund.Get(tx.Hash).Length > 0) return 3; //0.1

            if (!Runtime.CheckWitness(from)) return 4; //0.2
